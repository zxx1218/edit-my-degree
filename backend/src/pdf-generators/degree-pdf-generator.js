...
const generateDegreePdf = async (req, res) => {
  try {
    ...
    try {
      ...
      */
      ...
      if (qrCodeMode === 'available') {
        // 生成完整URL
        const fullUrl = `${process.env.VERIFICATION_BASE_URL}/verification-degree?${queryString}`;
        
        // 使用短码替代完整URL（有效期从环境变量QR_CODE_EXPIRES_IN_DAYS读取，默认1天）
        try {
          const shortCode = await qrManager.saveUrlWithShortCode(fullUrl, 'degree');
    ...
    logger.info('==========✅ 学位在线验证报告 PDF 处理完成==========');
  } catch (error) {
    logger.error(`❌ PDF generation error: ${error.message}`);
    res.status(500).json({
      success: false,
      error: '学位在线验证报告 PDF 生成失败：' + error.message
    });
  }
};
