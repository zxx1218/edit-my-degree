...
const generateStudentStatusPdf = async (req, res) => {
  try {
    ...
    logger.info('✅ 学籍在线验证报告PDF文档保存完成，大小:', pdfBytes.length, '字节');
    ...
    try {
      ...
      logger.info('✅ 学籍在线验证报告PDF文件已在后端保存:', filePath);
    } catch (saveError) {
      logger.error('❌ 保存学籍在线验证报告PDF到后端目录失败:', saveError.message);
      // 不中断流程，仍然发送给前端
    }
    ...
    logger.info('✅ 学籍在线验证报告PDF文件发送成功');
  } catch (error) {
    logger.error("❌ 学籍在线验证报告PDF生成错误:", error);
    res.status(500).json({
      success: false,
