...
function setupRoutes(app, db, JWT_SECRET) {
  ...
  // 初始化各接口模块
  app.post('/api/get-user-data', generalLimiter, getUserDataModule.initialize(db));
  app.post('/api/update-data', generalLimiter, updateDataModule.initialize(db));
  app.post('/api/update-user-logins', generalLimiter, updateUserLoginsModule.initialize(db));
  app.post('/api/change-password', generalLimiter, changePasswordModule.initialize(db));
  app.post('/api/reset-user-logins', generalLimiter, resetUserLoginsModule.initialize(db));
  app.post('/api/decrease-user-logins', generalLimiter, decreaseUserLoginsModule.initialize(db));
  app.post('/api/get-all-users', generalLimiter, getAllUsersModule.initialize(db, JWT_SECRET));
  app.post('/api/query-user', generalLimiter, queryUserModule.initialize(db));
  app.post('/api/decrease-pdf-limit', generalLimiter, decreasePdfLimitModule.initialize(db));
  app.post('/api/get-today-login-count', generalLimiter, getTodayLoginCountModule.initialize(db));
  app.post('/api/increase-pdf-limit', generalLimiter, increasePdfLimitModule.initialize(db));
  app.post('/api/reset-pdf-limit', generalLimiter, resetPdfLimitModule.initialize(db));
  app.post('/api/get-hourly-login-stats', generalLimiter, getHourlyLoginStatsModule.initialize(db));
  app.post('/api/get-login-stats-range', generalLimiter, getLoginStatsRangeModule.initialize(db));
  
  // 添加充值卡管理接口
  app.post('/api/manage-cards', generalLimiter, manageCards(db));
