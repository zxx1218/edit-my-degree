...
const { logIpBlacklist, logOperation } = require('./operation-logger');
...
function initialize(db, JWT_SECRET) {
  return async (req, res) => {
    try {
      ...
      const { action, type } = req.body; // type: 'ip' | 'user'
      ...
      if (type === 'ip' || !type) {
        ...
        if (action === 'update') {
          ...
          logOperation(null, decoded.username, 'updated', `更新IP黑名单记录: ${JSON.stringify({ id, blockedUntil, reason })}`, {
        ...
        if (action === 'delete') {
          ...
          logIpBlacklist(ipAddress, 'removed', `IP黑名单已移除，操作者: ${decoded.username}`);
        ...
        if (action === 'add') {
          ...
          logIpBlacklist(ipAddress, 'added', `IP已被加入黑名单，原因: ${reason}, 操作者: ${decoded.username}`, {
            adminUsername: decoded.username,
            ip: req.ip,
      ...
      console.error('[黑名单管理] 错误:', error.message, { stack: error.stack });
