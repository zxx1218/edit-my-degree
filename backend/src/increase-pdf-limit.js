  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (isad === true) {
        ...
        if (!adminToken) {
          console.warn(`[安全警告] PDF积分增加管理员操作缺少adminToken - IP: ${ipAddress}, User-Agent: ${userAgent}`);
        ...
        try {
          ...
          console.warn(`[安全警告] PDF积分增加管理员Token验证失败 - 错误: ${error.message}, IP: ${ipAddress}, User-Agent: ${userAgent}`);
