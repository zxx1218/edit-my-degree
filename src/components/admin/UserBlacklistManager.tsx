...
interface UserBlacklistManagerProps {
  token: string | null;
}

const ITEMS_PER_PAGE = 5;

const UserBlacklistManager = ({ token }: UserBlacklistManagerProps) => {
  ...
  const fetchBlacklist = async () => {
    ...
    setIsLoading(true);
    try {
      const data = await adminApi.getUserBlacklist(token);
      if (data.success) {
  ...
  const handleUpdate = async () => {
    ...
    setIsUpdating(true);
    try {
      const data = await adminApi.updateUserBlacklist(token, {
        id: selectedItem.id,
        reason: editReason,
      ...
      if (data.success) {
        toast({
          title: "更新成功",
          description: `已更新用户 ${selectedItem.username} 的黑名单记录`,
        });
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const data = await adminApi.deleteUserBlacklist(token, selectedItem.id);

      if (data.success) {
        toast({
          title: "删除成功",
          description: `已删除用户 ${selectedItem.username} 的黑名单记录`,
        });
  ...
  const handleAdd = async () => {
    ...
    setIsAdding(true);
    try {
      const data = await adminApi.addUserBlacklist(token, {
      ...
      if (data.success) {
        toast({
          title: "添加成功",
          description: `已将用户 ${addUsername} 加入黑名单`,
        });
      ...
      <CardHeader>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <CardTitle className="text-xl">用户黑名单管理</CardTitle>
              <CardDescription>查看和管理被封禁的用户账号</CardDescription>
            </div>
          </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 space-y-2 mb-3 sm:mb-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-semibold text-red-600 dark:text-red-400 break-all">
                        {item.username}
                      </span>
                      <Badge
                    ...
                    <div className="text-xs text-muted-foreground space-y-1">
                      ...
                      <p className="hidden sm:block">
                        <span className="font-medium">操作者:</span> {item.createdBy}
                      </p>
                      <p className="hidden sm:block">
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              修改用户 {selectedItem?.username} 的封禁信息
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              此操作将永久删除用户 {selectedItem?.username} 的黑名单记录，该用户可以重新登录系统。
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2 sm:justify-end">
            <Button
...
export default UserBlacklistManager;
