...
function checkBlacklistResponse(response: Response, data: any): boolean {
  ...
  if (response.status === 403 && (data.error || data.message)) {
    // 优先检查 isBlacklisted 标识字段
    const isBlacklisted = data.isBlacklisted === true;
    
    // 如果没有标识字段，则通过关键词检测
    let shouldShowAlert = isBlacklisted;
    
    if (!shouldShowAlert) {
      const errorMsg = data.message || data.error;
      // 检测黑名单相关的错误信息
      if (errorMsg.includes('封禁') || 
          errorMsg.includes('拉黑') || 
          errorMsg.includes('黑名单') ||
          errorMsg.includes('机器码') ||
    ...
    if (shouldShowAlert) {
      // 使用toast显示封禁原因
      showBlacklistAlert(data.message || data.error);
      return true;
    }
  }
...
async function fetchWithBlacklistCheck(url: string, options?: RequestInit): Promise<Response> {
  ...
  try {
    ...
    // 如果不是JSON响应，尝试以文本方式读取
    try {
      const text = await clonedResponse.text();
      if (text) {
        checkBlacklistResponse(response, { error: text, message: text });
      }
    } catch (textErr) {
      // 如果连文本都读取失败，忽略
      console.warn('无法读取响应内容:', textErr);
    }
  }
